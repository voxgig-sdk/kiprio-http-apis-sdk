...
class DomainEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = KiprioHttpApisSDK.test(seed, nil)
    seen = base.Domain(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = KiprioHttpApisConfig.shared_config
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set KIPRIO_HTTP_APIS_TEST_DOMAIN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def domain_basic_setup(extra)
  ...
  entid_env_raw = ENV["KIPRIO_HTTP_APIS_TEST_DOMAIN_ENTID"]
