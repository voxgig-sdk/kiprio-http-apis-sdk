...
class DomainEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set KIPRIOHTTPAPIS_TEST_DOMAIN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def domain_basic_setup(extra)
  ...
  entid_env_raw = ENV["KIPRIOHTTPAPIS_TEST_DOMAIN_ENTID"]
  idmap_overridden = !entid_env_raw.nil? && entid_env_raw.strip.start_with?("{")

  env = Runner.env_override({
    "KIPRIOHTTPAPIS_TEST_DOMAIN_ENTID" => idmap,
  ...
  idmap_resolved = Helpers.to_map(
    env["KIPRIOHTTPAPIS_TEST_DOMAIN_ENTID"])
