...
require_relative 'KiprioHttpApis_types'
...
class KiprioHttpApisSDK
  ...
  def initialize(options = {})
    ...
    utility = KiprioHttpApisUtility.new
    @_utility = utility
    ...
    feature_opts = KiprioHttpApisHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = KiprioHttpApisHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, KiprioHttpApisFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    KiprioHttpApisUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = KiprioHttpApisHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = KiprioHttpApisHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = KiprioHttpApisHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue KiprioHttpApisError => err
    ...
    if fetched.is_a?(Hash)
      status = KiprioHttpApisHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = KiprioHttpApisError.new(
        "graphql_error", "KiprioHttpApisSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = KiprioHttpApisSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
