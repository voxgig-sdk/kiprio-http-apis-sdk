...
module KiprioHttpApisUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = KiprioHttpApisResponse.new({})
    ...
    if fetch_err
      ...
      response = KiprioHttpApisResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = KiprioHttpApisResponse.new(fetched)
    else
