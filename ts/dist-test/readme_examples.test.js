...
const __1 = require("..");
const SDK_NAME = 'KiprioHttpApisSDK';
...
function isRunnable(code) {
    return (/new\s+KiprioHttpApisSDK\b/.test(code) ||
        /\bKiprioHttpApisSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+KiprioHttpApisSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/KiprioHttpApisSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
