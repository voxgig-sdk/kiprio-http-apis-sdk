...
// KiprioHttpApis SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class KiprioHttpApisTestFeature extends KiprioHttpApisBaseFeature
{
    ...
    public function init(KiprioHttpApisContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (KiprioHttpApisContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = KiprioHttpApisParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (KiprioHttpApisContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(KiprioHttpApisContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = KiprioHttpApisParam::call($ctx, $k);
