...
// KiprioHttpApis SDK pipeline test
//
...
{
    private static function utility(): KiprioHttpApisUtility
    {
        return new KiprioHttpApisUtility();
    }
    ...
    private static function ctx(array $over = []): KiprioHttpApisContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new KiprioHttpApisContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new KiprioHttpApisOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof KiprioHttpApisError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new KiprioHttpApisOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = KiprioHttpApisMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new KiprioHttpApisOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = KiprioHttpApisMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new KiprioHttpApisOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = KiprioHttpApisMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new KiprioHttpApisSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = KiprioHttpApisMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new KiprioHttpApisResponse([]),
            'result' => new KiprioHttpApisResult([]),
        ]);
        [, $err] = KiprioHttpApisMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new KiprioHttpApisSpec([]),
            'response' => null,
            'result' => new KiprioHttpApisResult([]),
        ]);
        [, $err] = KiprioHttpApisMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new KiprioHttpApisSpec(['step' => 's']),
            'response' => new KiprioHttpApisResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new KiprioHttpApisResult([]),
        ]);
        [, $err] = KiprioHttpApisMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new KiprioHttpApisSpec(['step' => 's']),
            'response' => new KiprioHttpApisResponse(self::resp(200, ['v' => 1])),
            'result' => new KiprioHttpApisResult([]),
        ]);
        [, $err] = KiprioHttpApisMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new KiprioHttpApisSpec(['step' => 's']),
            'response' => new KiprioHttpApisResponse(self::resp(200, ['v' => 2])),
            'result' => new KiprioHttpApisResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        KiprioHttpApisMakeResponse::call($ctx);
    ...
    {
        $preset = new KiprioHttpApisResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new KiprioHttpApisSpec([]),
            'response' => new KiprioHttpApisResponse([]),
            'result' => new KiprioHttpApisResult([]),
        ...
        [$got, $err] = KiprioHttpApisMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new KiprioHttpApisResult([])]);
        [, $err] = KiprioHttpApisMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new KiprioHttpApisSpec([]), 'result' => null]);
        [, $err] = KiprioHttpApisMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new KiprioHttpApisSpec(['step' => 's']),
            'result' => new KiprioHttpApisResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new KiprioHttpApisOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = KiprioHttpApisMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new KiprioHttpApisSpec(['step' => 's']),
            'result' => new KiprioHttpApisResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new KiprioHttpApisOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = KiprioHttpApisMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new KiprioHttpApisResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new KiprioHttpApisSpec([]),
            'result' => new KiprioHttpApisResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = KiprioHttpApisMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = KiprioHttpApisMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new KiprioHttpApisError('boom', 'boom');
        $utility->fetcher = function (KiprioHttpApisContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new KiprioHttpApisSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = KiprioHttpApisMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (KiprioHttpApisContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = KiprioHttpApisMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(KiprioHttpApisResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        KiprioHttpApisMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (KiprioHttpApisContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new KiprioHttpApisSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = KiprioHttpApisMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new KiprioHttpApisResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new KiprioHttpApisSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = KiprioHttpApisMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = KiprioHttpApisMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = KiprioHttpApisMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new KiprioHttpApisResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, KiprioHttpApisDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new KiprioHttpApisResult(['ok' => false])]);
        $this->expectException(KiprioHttpApisError::class);
        KiprioHttpApisDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new KiprioHttpApisResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', KiprioHttpApisMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        KiprioHttpApisMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new KiprioHttpApisBaseFeature();
        $b = new KiprioHttpApisBaseFeature();
        KiprioHttpApisFeatureAdd::call($ctx, $a);
        KiprioHttpApisFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): KiprioHttpApisBaseFeature
    {
        $f = new KiprioHttpApisBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        KiprioHttpApisFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): KiprioHttpApisContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new KiprioHttpApisSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = KiprioHttpApisPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        KiprioHttpApisPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new KiprioHttpApisResponse(['headers' => null]),
            'result' => new KiprioHttpApisResult([]),
        ]);
        KiprioHttpApisResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        KiprioHttpApisResultBody::call($ctx);
