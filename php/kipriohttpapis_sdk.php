...
class KiprioHttpApisSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = KiprioHttpApisHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = KiprioHttpApisHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!KiprioHttpApisFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, KiprioHttpApisFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return KiprioHttpApisUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = KiprioHttpApisHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = KiprioHttpApisHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = KiprioHttpApisHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new KiprioHttpApisError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = KiprioHttpApisHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new KiprioHttpApisError("graphql_error",
                "KiprioHttpApisSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new KiprioHttpApisSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
