...
//    only - a typed KiprioHttpApisContext there would make every ctx
...
final class KiprioHttpApisOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new KiprioHttpApisContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new KiprioHttpApisSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new KiprioHttpApisResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new KiprioHttpApisError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new KiprioHttpApisResponse($clean['response']);
    ...
    public static function err_from_map($m): ?KiprioHttpApisError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new KiprioHttpApisError(is_string($code) ? $code : '', $msg);
    }
