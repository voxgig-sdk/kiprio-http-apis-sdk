...
// KiprioHttpApis SDK feature test
//
...
class FtCtrl extends KiprioHttpApisControl
{
...
class FtClient
{
    ...
    private KiprioHttpApisUtility $utility;

    public function __construct(KiprioHttpApisUtility $utility, array $options)
    {
    ...
    public function get_utility(): KiprioHttpApisUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (KiprioHttpApisContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public KiprioHttpApisContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (KiprioHttpApisContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = KiprioHttpApisConfig::make_feature($name);
    ...
    private static function buildUrl(KiprioHttpApisSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof KiprioHttpApisError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new KiprioHttpApisResponse($fetched);
        }
    ...
    private function fail(KiprioHttpApisContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (KiprioHttpApisContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
