...
class KiprioHttpApisMakeError
{
    public static function call(?KiprioHttpApisContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new KiprioHttpApisContext([], null);
        }
        $op = $ctx->op ?? new KiprioHttpApisOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new KiprioHttpApisResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof KiprioHttpApisError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new KiprioHttpApisError('', $msg, $ctx);
        ...
        if ($err instanceof KiprioHttpApisError) {
