...
// KiprioHttpApis SDK retry feature
...
class KiprioHttpApisRetryFeature extends KiprioHttpApisBaseFeature
{
    ...
    public function init(KiprioHttpApisContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (KiprioHttpApisContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(KiprioHttpApisContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(KiprioHttpApisContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
