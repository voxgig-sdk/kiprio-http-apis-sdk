...
func TestUtilityEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set KIPRIOHTTPAPIS_TEST_UTILITY_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func utilityBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("KIPRIOHTTPAPIS_TEST_UTILITY_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"KIPRIOHTTPAPIS_TEST_UTILITY_ENTID": idmap,
		"KIPRIOHTTPAPIS_TEST_LIVE":      "FALSE",
		"KIPRIOHTTPAPIS_TEST_EXPLAIN":   "FALSE",
		"KIPRIOHTTPAPIS_APIKEY":         "NONE",
	})

	idmapResolved := core.ToMapAny(env["KIPRIOHTTPAPIS_TEST_UTILITY_ENTID"])
