...
import (
	...
	omni "github.com/voxgig-sdk/kiprio-http-apis-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.KiprioHttpApisError); is {
				out := map[string]any{
					"name":    "KiprioHttpApisError",
...
func omniCtx(arg any, client *sdk.KiprioHttpApisSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.KiprioHttpApisSDK); is {
			client = live
