// Command KiprioHttpApis-mcp is an MCP server exposing the KiprioHttpApis
// SDK as MCP tools.
//
// Two tools are registered:
//
//	KiprioHttpApis_list  — list records of an entity
//	KiprioHttpApis_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: KIPRIO_HTTP_APIS_APIKEY carries the API key and
	// KIPRIO_HTTP_APIS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("KIPRIO_HTTP_APIS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("KIPRIO_HTTP_APIS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewKiprioHttpApisSDK(opts)
