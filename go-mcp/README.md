# kiprio-http-apis-mcp

[MCP](https://modelcontextprotocol.io) server exposing the KiprioHttpApis SDK as
two agent tools — `kiprio-http-apis_list` and `kiprio-http-apis_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kiprio-http-apis-mcp)
make build

# 2. Provide credentials via the environment
export KIPRIO_HTTP_APIS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user kiprio-http-apis \
  -- /absolute/path/to/kiprio-http-apis-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./kiprio-http-apis-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// kiprio-http-apis_list: first page of records
{ "entity": "domain" }
{ "entity": "domain", "query": { } }

// kiprio-http-apis_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kiprio-http-apis-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user kiprio-http-apis \
     -- "$PWD"/dist/*/kiprio-http-apis-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `kiprio-http-apis_list` and `kiprio-http-apis_load` tools now appear
   in new sessions. Ask the agent to *"list domain using kiprio-http-apis"*
   and it calls `kiprio-http-apis_list` with `{"entity":"domain"}`.
...
```sh
export KIPRIO_HTTP_APIS_APIKEY=sk_live_xxx            # API key
export KIPRIO_HTTP_APIS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `kiprio-http-apis_list` tool
...
### Call the `kiprio-http-apis_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `kiprio-http-apis_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `kiprio-http-apis_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `KIPRIO_HTTP_APIS_APIKEY` | API key sent with every request. |
| `KIPRIO_HTTP_APIS_BASE` | Optional override of the API base URL. |
...
```sh
./kiprio-http-apis-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"kiprio-http-apis_load","arguments":{"entity":"dns_result","query":{"id":1}}}}'
```
