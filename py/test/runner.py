# KiprioHttpApis SDK test runner
...
class KiprioHttpApisTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            KiprioHttpApisTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = KiprioHttpApisTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = KiprioHttpApisTestRunner.getenv("KIPRIO_HTTP_APIS_TEST_LIVE")
        override = KiprioHttpApisTestRunner.getenv("KIPRIO_HTTP_APIS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = KiprioHttpApisTestRunner.getenv(key)
        ...
        explain = KiprioHttpApisTestRunner.getenv("KIPRIO_HTTP_APIS_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if KiprioHttpApisTestRunner._test_control is not None:
            return KiprioHttpApisTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                KiprioHttpApisTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            KiprioHttpApisTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = KiprioHttpApisTestRunner.load_test_control()
...
def load_env_local():
    KiprioHttpApisTestRunner.load_env_local()


def env_override(m):
    return KiprioHttpApisTestRunner.env_override(m)


def entity_data(v):
    return KiprioHttpApisTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return KiprioHttpApisTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return KiprioHttpApisTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return KiprioHttpApisTestRunner.load_test_control()


def live_client_options():
    return KiprioHttpApisTestRunner.live_client_options()


def live_delay_ms():
    return KiprioHttpApisTestRunner.live_delay_ms()
