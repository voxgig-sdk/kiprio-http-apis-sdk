# kiprio-http-apis-cli

boru-driven command-line client **and** interactive REPL for the KiprioHttpApis
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kiprio-http-apis-cli)
make build

# 2. See usage (words, entities, env vars)
./kiprio-http-apis-cli --help

# 3. Provide credentials once, via the environment
export KIPRIO_HTTP_APIS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./kiprio-http-apis-cli load 1 dns_result            # {id:1} shorthand
./kiprio-http-apis-cli load '{id:1}' dns_result       # explicit match map

# 5. Override the API base URL for a single call
KIPRIO_HTTP_APIS_BASE=https://api.example.com ./kiprio-http-apis-cli load 1 dns_result

# 6. No arguments -> interactive REPL
./kiprio-http-apis-cli
kiprio-http-apis> load 1 dns_result
kiprio-http-apis> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kiprio-http-apis-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/kiprio-http-apis-cli load 1 dns_result
   ```
...
```sh
./kiprio-http-apis-cli load 1 dns_result          # scalar shorthand for {id:1}
./kiprio-http-apis-cli load '{id:1}' dns_result     # explicit match map
```
...
./kiprio-http-apis-cli load 1 dns_result
```
...
Run with no arguments to open a REPL (prompt `kiprio-http-apis>`). Each line is
evaluated as its own boru expression:

```text
$ ./kiprio-http-apis-cli
kiprio-http-apis> load 1 dns_result
kiprio-http-apis> /help
kiprio-http-apis> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/kiprio-http-apis-cli`. |
...
(`./kiprio-http-apis-cli <expr>`) and interactively (the REPL), and expressions compose the
